...
namespace Match3
{
    public class GridSystem2D<T>
    {
        ...
        public GridSystem2D(int width, int height, float cellSize, Vector3 origin, CoordinateConverter coordinateConverter, bool debug)
        {
            ...
            this.origin = origin;
            this.coordinateConverter = coordinateConverter ?? new VerticalConverter();//??:如果是空 就执行后面的
        ...
        public T GetValue(Vector3 worldPositon)
        {
        ...
        public Vector2Int GetXY(Vector3 worldPostion) => coordinateConverter.WorldToGrid(worldPostion, cellSize, origin);
        ...
        public abstract class CoordinateConverter
        {
            ...
            public abstract Vector2Int WorldToGrid(Vector3 worldPositon, float cellSize, Vector3 origin);
        ...
        public class VerticalConverter : CoordinateConverter
        {
            ...
            public override Vector2Int WorldToGrid(Vector3 worldPositon, float cellSize, Vector3 origin)
            {
                Vector3 gridPostion = (worldPositon - origin) / cellSize;
                var x = Mathf.FloorToInt(gridPostion.x); //返回<=的最大整数  例如2.8  会返回2
                var y = Mathf.FloorToInt(gridPostion.y);
        ...
        public class HorizontalConverter : CoordinateConverter
        {
            ...
            public override Vector2Int WorldToGrid(Vector3 worldPositon, float cellSize, Vector3 origin)
            {
                Vector3 gridPostion = (worldPositon - origin) / cellSize;
                var x = Mathf.FloorToInt(gridPostion.x); //返回<=的最大整数  例如2.8  会返回2
                var y = Mathf.FloorToInt(gridPostion.z);
                return new Vector2Int(x, y);
            }
        }
    }
